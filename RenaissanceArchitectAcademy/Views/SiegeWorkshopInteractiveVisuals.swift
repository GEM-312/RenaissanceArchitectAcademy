        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    DimLabel(text: "300m range", fontSize: 9).position(x: w * 0.75, y: baseY + 8)
                    if step >= 3 { FormulaText(text: "E = ½kx² — elastic energy", highlighted: true, fontSize: 11).position(x: w * 0.5, y: h * 0.88).transition(.opacity) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    // Range label
                    DimLabel(text: "\(Int(range / w * 300))m", fontSize: 10).position(x: cx + range * 0.5, y: baseY + 14)
                    if step >= 2 {
                    ...
                    if step >= 3 { FormulaText(text: "45° = maximum range", highlighted: true, fontSize: 12).position(x: w * 0.5, y: h * 0.12).transition(.opacity) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    if isDangerous { Text("⚠️ SNAP RISK").font(.custom("Cinzel-Bold", size: 10)).foregroundStyle(RenaissanceColors.errorRed).position(x: cx, y: h * 0.22) }
                    if step >= 2 {
                    ...
                    if step >= 3 { FormulaText(text: "2× twists = 4× power (square law)", highlighted: true, fontSize: 11).position(x: cx, y: h * 0.88).transition(.opacity) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    // Force counter
                    Text("Force: \(swingCount * 6) men").font(.custom("EBGaramond-Bold", size: 12)).foregroundStyle(sepiaInk).position(x: cx, y: h * 0.78)
                    // Tap area
                    if step >= 2 && swingCount < 5 {
                        Button {
                            ...
                            Text("SWING").font(.custom("Cinzel-Bold", size: 14)).foregroundStyle(color)
                    ...
                    if step >= 3 { FormulaText(text: "Rhythm beats raw strength", highlighted: true, fontSize: 11).position(x: cx, y: h * 0.9).transition(.opacity) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    Text("Wall").font(.custom("EBGaramond-Regular", size: 7)).foregroundStyle(sepiaInk.opacity(0.3)).position(x: w * 0.82, y: h * 0.15)
                    // Tower levels
                    ...
                    if step >= 2 && levelsBuilt < 6 {
                        ...
                        } label: { Text("Build \(levels[levelsBuilt])").font(.custom("EBGaramond-SemiBold", size: 12)).foregroundStyle(color)
                    ...
                    DimLabel(text: "20m", fontSize: 9).position(x: cx - towerW * 0.5 - 14, y: h * 0.4)
                    if step >= 3 { FormulaText(text: "Engineering under maximum pressure", highlighted: true, fontSize: 10).position(x: cx, y: h * 0.88).transition(.opacity) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in let w = geo.size.width; let h = geo.size.height; let cx = w * 0.5
                ZStack {
                    ...
                    if vitrified { FormulaText(text: "500 tiles × 2kg = 1 ton of fire armor", highlighted: true, fontSize: 10).position(x: cx, y: h*0.82) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in let w = geo.size.width; let h = geo.size.height; let cx = w * 0.5
                ZStack {
                    ...
                    Text(isSmelting ? "Iron bloom" : tempC > 600 ? "Heating..." : "Iron ore").font(.custom("EBGaramond-Regular", size: 10)).foregroundStyle(sepiaInk.opacity(0.6)).position(x: cx, y: h * 0.52)
                    ...
                    if isSmelting { FormulaText(text: "10 folds = 1,024 layers", highlighted: true, fontSize: 11).position(x: cx, y: h*0.85) }
                }
            }
        }
    }
                ...
                ZStack {
                    ...
                    HStack(spacing: 16) {
                        Button { guard !copperAdded else { return }; withAnimation(.spring(response: 0.3)) { copperAdded = true }; SoundManager.shared.play(.tapSoft) } label: {
                            VStack { Text("90%").font(.custom("EBGaramond-Bold", size: 14)); Text("Copper").font(.custom("EBGaramond-Regular", size: 9)) }
                        ...
                        Button { guard copperAdded && !tinAdded else { return }; withAnimation(.spring(response: 0.3)) { tinAdded = true }; SoundManager.shared.play(.tapSoft)
                            ...
                            VStack { Text("10%").font(.custom("EBGaramond-Bold", size: 14)); Text("Tin").font(.custom("EBGaramond-Regular", size: 9)) }
                    ...
                    if step >= 3 { FormulaText(text: "Precision starts in wax", highlighted: true, fontSize: 11).position(x: cx, y: h * 0.85).transition(.opacity) }
                }
            }
        }
    }
            ...
            GeometryReader { geo in let w = geo.size.width; let h = geo.size.height
                ZStack {
                    HStack(spacing: w * 0.06) {
                        VStack(spacing: 4) {
                            Text("SOAKED").font(.custom("Cinzel-Bold", size: 9)).foregroundStyle(sepiaInk.opacity(0.5))
                            ...
                            if tested { Text("Flexes").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.sageGreen) }
                        }
                        VStack(spacing: 4) {
                            Text("DRY").font(.custom("Cinzel-Bold", size: 9)).foregroundStyle(sepiaInk.opacity(0.5))
                            ZStack {
                            ...
                            if tested { Text("Shatters").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.errorRed) }
                        }
                    }.position(x: w * 0.5, y: h * 0.4)
                    if step >= 2 && !tested {
                        Button { withAnimation(.spring(response: 0.3)) { tested = true }; SoundManager.shared.play(.tapSoft)
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { withAnimation { step = 3 } }
                        } label: { HStack(spacing: 4) { Image(systemName: "hammer.fill").font(.system(size: 10)); Text("Impact Test").font(.custom("EBGaramond-SemiBold", size: 12)) }
                    ...
                    if step >= 3 { FormulaText(text: "Wet absorbs shock — dry transfers it", highlighted: true, fontSize: 11).position(x: w*0.5, y: h*0.85).transition(.opacity) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in let w = geo.size.width; let h = geo.size.height
                ZStack {
                    HStack(spacing: w * 0.06) {
                        VStack(spacing: 4) {
                            Text("GREEN OAK").font(.custom("Cinzel-Bold", size: 8)).foregroundStyle(sepiaInk.opacity(0.5))
                            RoundedRectangle(cornerRadius: 4).fill(RenaissanceColors.sageGreen.opacity(0.3)).frame(width: w*0.25, height: h*0.2)
                            Text("500 shots").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.sageGreen)
                            Text("Flexible").font(.custom("EBGaramond-Regular", size: 8)).foregroundStyle(sepiaInk.opacity(0.4))
                        }
                        VStack(spacing: 4) {
                            Text("DRY OAK").font(.custom("Cinzel-Bold", size: 8)).foregroundStyle(sepiaInk.opacity(0.5))
                            RoundedRectangle(cornerRadius: 4).fill(oakBrown.opacity(0.3)).frame(width: w*0.25, height: h*0.2)
                            Text("50 shots").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.errorRed)
                            Text("Brittle").font(.custom("EBGaramond-Regular", size: 8)).foregroundStyle(sepiaInk.opacity(0.4))
                        }
                    }.position(x: w * 0.5, y: h * 0.4)
                    if step >= 2 { FormulaText(text: "10× lifespan — moisture = flexibility", highlighted: true, fontSize: 11).position(x: w*0.5, y: h*0.75) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in let w = geo.size.width; let h = geo.size.height
                ZStack {
                    HStack(spacing: w * 0.06) {
                        VStack(spacing: 4) {
                            Text("WALNUT").font(.custom("Cinzel-Bold", size: 9)).foregroundStyle(sepiaInk.opacity(0.5))
                            ...
                            if tested { Text("Stable").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.sageGreen) }
                        }
                        VStack(spacing: 4) {
                            Text("OTHER").font(.custom("Cinzel-Bold", size: 9)).foregroundStyle(sepiaInk.opacity(0.5))
                            RoundedRectangle(cornerRadius: 4).fill(oakBrown.opacity(0.3)).frame(width: w*0.25, height: h*0.2)
                                .overlay { if tested { Image(systemName: "xmark.circle.fill").foregroundStyle(RenaissanceColors.errorRed.opacity(0.5)) } }
                            if tested { Text("Swells").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.errorRed) }
                        }
                    }.position(x: w * 0.5, y: h * 0.38)
                    if step >= 2 && !tested {
                        Button { withAnimation(.spring(response: 0.3)) { tested = true }; SoundManager.shared.play(.tapSoft)
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { withAnimation { step = 3 } }
                        } label: { Text("Oil Test").font(.custom("EBGaramond-SemiBold", size: 12)).foregroundStyle(color)
                    ...
                    if step >= 3 { FormulaText(text: "The wood that doesn't change = the one you trust", highlighted: true, fontSize: 10).position(x: w*0.5, y: h*0.85).transition(.opacity) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in let w = geo.size.width; let h = geo.size.height; let cx = w * 0.5
                ZStack {
                        ...
                        HStack(spacing: 8) {
                            ...
                            if revealed {
                                VStack(alignment: .leading, spacing: 1) {
                                    Text(joints[i].0).font(.custom("Cinzel-Bold", size: 8)).foregroundStyle(sepiaInk)
                                    Text(joints[i].1).font(.custom("EBGaramond-Regular", size: 7)).foregroundStyle(sepiaInk.opacity(0.5))
                                }.transition(.opacity)
                            }
                        }.position(x: cx, y: y)
                            .onTapGesture {
                    ...
                    if step >= 3 { FormulaText(text: "Modularity before the word existed", highlighted: true, fontSize: 11).position(x: cx, y: h*0.85).transition(.opacity) }
                }
            }
        }
    }
            ...
            GeometryReader { geo in let w = geo.size.width; let h = geo.size.height; let cx = w * 0.5
                ZStack {
                    ...
                    HStack(spacing: 8) {
                        ForEach(0..<3, id: \.self) { i in
                            VStack(spacing: 2) {
                                ...
                                Text(phases[i].0).font(.custom("EBGaramond-Regular", size: 7)).foregroundStyle(i < phase ? sepiaInk : sepiaInk.opacity(0.3))
                            }
                        }
                    ...
                    if step >= 2 && phase < 3 {
                        ...
                        } label: { Text(phases[phase].0).font(.custom("EBGaramond-SemiBold", size: 12)).foregroundStyle(phases[phase].2)
                    ...
                    if step >= 3 { FormulaText(text: "Hard to pierce + tough not to shatter", highlighted: true, fontSize: 10).position(x: cx, y: h*0.88).transition(.opacity) }
                }
            }
        }
    }
