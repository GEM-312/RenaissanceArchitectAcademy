            ...
            VStack(spacing: 10) {
                HStack(spacing: 20) {
                    VStack(spacing: 4) {
                        ...
                        Text("1 Bible").font(RenaissanceFont.ivFormula).foregroundStyle(step >= 1 ? IVMaterialColors.sepiaInk : IVMaterialColors.sepiaInk.opacity(0.3))
                        Text("2 years").font(RenaissanceFont.ivBody).foregroundStyle(IVMaterialColors.dimColor.opacity(step >= 1 ? 0.6 : 0.2))
                    }
                    if step >= 2 {
                        Image(systemName: "arrow.right").font(.system(size: 13)).foregroundStyle(IVMaterialColors.sepiaInk.opacity(0.3))
                        VStack(spacing: 4) {
                            Text("🖨️").font(.system(size: 20))
                            Text("180 Bibles").font(RenaissanceFont.ivFormula).foregroundStyle(color)
                            Text("3 years").font(RenaissanceFont.ivBody).foregroundStyle(IVMaterialColors.dimColor)
                        }
                    }
                }
                if step >= 3 {
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            Canvas { ctx, size in
                ...
                if step >= 1 {
                    ...
                    ctx.stroke(handle, with: .color(IVMaterialColors.oakBrown.opacity(0.5)), lineWidth: 3)
                }
                // Platen
                let platenY = cy + 30
    ...
    var body: some View {
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            VStack(spacing: 8) {
                    ...
                    HStack(spacing: 1) {
                        ForEach(Array(metals.enumerated()), id: \.offset) { i, metal in
                            ...
                            Rectangle()
                                ...
                                .overlay {
                                    VStack(spacing: 1) {
                                        Text(metal.1).font(RenaissanceFont.ivFormula)
                                        Text(metal.0).font(RenaissanceFont.ivBody)
                ...
                if step >= 3 { Text("+1% expansion on cooling").font(RenaissanceFont.ivFormula).foregroundStyle(color) }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            VStack(spacing: 8) {
                ...
                if step >= 1 {
                    ...
                    Text("← mirrored").font(RenaissanceFont.ivBody).foregroundStyle(IVMaterialColors.dimColor.opacity(0.5))
                }
                if step >= 2 { Text("1,500 chars/hour").font(RenaissanceFont.ivFormula).foregroundStyle(color) }
                if step >= 3 { Text("→ 500 copies/day").font(RenaissanceFont.ivFormula).foregroundStyle(color) }
            }
        }
    }
}
    ...
    var body: some View {
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            VStack(spacing: 6) {
                ...
                if step >= 3 { Text("500 impressions per piece").font(RenaissanceFont.ivFormula).foregroundStyle(color) }
            }
        }
    }
    ...
    var body: some View {
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            VStack(spacing: 8) {
                ...
                if step >= 2 { Text("20-25% moisture").font(RenaissanceFont.ivFormula).foregroundStyle(color) }
                if step >= 3 {
                    HStack(spacing: 20) {
                        Text("Too wet: bleed").font(RenaissanceFont.ivBody).foregroundStyle(.red.opacity(0.5))
                        Text("Too dry: smear").font(RenaissanceFont.ivBody).foregroundStyle(.red.opacity(0.5))
                    }
                }
            }
        }
...
private struct IronFrameVisual: View {
    ...
    var body: some View {
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            .overlay(alignment: .bottom) {
                if step >= 3 { Text("3,000+ kg").font(RenaissanceFont.ivFormula).foregroundStyle(color).offset(y: -28) }
            }
        }
    }
}
    ...
    var body: some View {
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            VStack(spacing: 8) {
                HStack(spacing: 20) {
                    VStack(spacing: 4) {
                        RoundedRectangle(cornerRadius: 3).fill(IVMaterialColors.oakBrown.opacity(step >= 1 ? 0.5 : 0.15))
                            .frame(width: 25, height: 60)
                        Text("15×15cm").font(RenaissanceFont.ivFormula).foregroundStyle(step >= 1 ? IVMaterialColors.dimColor : IVMaterialColors.dimColor.opacity(0.3))
                    }
                    VStack(spacing: 4) {
                        RoundedRectangle(cornerRadius: 3).fill(IVMaterialColors.oakBrown.opacity(step >= 1 ? 0.5 : 0.15))
                            .frame(width: 25, height: 60)
                ...
                if step >= 2 { Text("1,500 kg each").font(RenaissanceFont.ivFormula).foregroundStyle(color) }
                if step >= 3 { Text("Bronze bushing → smooth screw").font(RenaissanceFont.ivBody).foregroundStyle(IVMaterialColors.dimColor) }
            }
        }
    }
}
            ...
            VStack(spacing: 6) {
                ...
                if step >= 2 {
                    RoundedRectangle(cornerRadius: 3).fill(walnutDark.opacity(0.25))
                        ...
                        .overlay(alignment: .leading) { Text("UPPER").font(RenaissanceFont.ivBody).foregroundStyle(IVMaterialColors.dimColor).padding(.leading, 4) }
                        .padding(.horizontal, 20)
                }
                // Lower case
                if step >= 1 {
                        ...
                        .overlay {
                            HStack(spacing: 2) {
                                ForEach(["a","b","c","d","e","f","g","h"], id: \.self) { ch in
                                    Text(ch).font(RenaissanceFont.ivBody).foregroundStyle(IVMaterialColors.sepiaInk.opacity(0.5))
                                        .frame(width: 14, height: 14)
                                        .background(RoundedRectangle(cornerRadius: 1).fill(paperCream.opacity(0.3)))
                                }
                            }
                        }
                        .overlay(alignment: .leading) { Text("lower").font(RenaissanceFont.ivBody).foregroundStyle(IVMaterialColors.dimColor).padding(.leading, 4) }
                        .padding(.horizontal, 20)
                }
                if step >= 3 { Text("\"Uppercase\" = top tray position").font(RenaissanceFont.ivFormula).foregroundStyle(color) }
            }
        }
    }
}
