        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                        ...
                        ForEach(0..<4, id: \.self) { i in
                            ...
                            if placed {
                                ...
                                Circle()
                                    ...
                                    .overlay { Text("+").font(.system(size: 12)).foregroundStyle(color.opacity(0.4)) }
                                    .position(positions[i])
                                    .onTapGesture {
                    ...
                    if step >= 3 {
                        FormulaText(text: "350 ships — Rome's lifeline", highlighted: true, fontSize: 11)
                ...
                ZStack {
                    ...
                    if waveHeight > 0.2 {
                        Image(systemName: "arrow.right")
                            .font(.system(size: 10 + waveHeight * 12))
                    ...
                    if step >= 3 {
                        FormulaText(text: "3m wave = 30 tons/meter", highlighted: true, fontSize: 11)
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 4,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    if step >= 2 && waterDrained > 0.2 {
                        Button {
                            ...
                            HStack(spacing: 4) {
                                ...
                                Text("Pump Water").font(.custom("EBGaramond-SemiBold", size: 12))
                            }
                    ...
                    if step >= 4 {
                        FormulaText(text: "Build on land — inside the sea", highlighted: true, fontSize: 11)
                            .position(x: cx, y: h * 0.88)
                            .transition(.opacity)
                    }
                }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    if step >= 2 && blocksPlaced < 4 {
                        Button {
                            ...
                            HStack(spacing: 4) {
                                ...
                                Text("Place Block").font(.custom("EBGaramond-SemiBold", size: 12))
                            }
                    ...
                    if step >= 3 {
                        FormulaText(text: "3× overbuilt = survives any storm", highlighted: true, fontSize: 11)
                            .position(x: cx, y: h * 0.88)
                            .transition(.opacity)
                    }
                }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    if step >= 3 {
                        FormulaText(text: "Fire + bronze mirror = 50 km visible", highlighted: true, fontSize: 11)
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    // Tufa block (left)
                    VStack(spacing: 4) {
                        Text("TUFA").font(.custom("Cinzel-Bold", size: 9)).tracking(0.5).foregroundStyle(sepiaInk.opacity(0.5))
                        ZStack {
                        ...
                        if saltPoured {
                            Text("Absorbs safely").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.sageGreen).transition(.opacity)
                        }
                        Text("Porous").font(.custom("EBGaramond-Regular", size: 8)).foregroundStyle(sepiaInk.opacity(0.4))
                    }
                    .position(x: w * 0.28, y: h * 0.4)

                    // Marble block (right)
                    VStack(spacing: 4) {
                        Text("MARBLE").font(.custom("Cinzel-Bold", size: 9)).tracking(0.5).foregroundStyle(sepiaInk.opacity(0.5))
                        ZStack {
                        ...
                        if saltPoured {
                            Text("Cracks inside").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.errorRed).transition(.opacity)
                        }
                        Text("Dense").font(.custom("EBGaramond-Regular", size: 8)).foregroundStyle(sepiaInk.opacity(0.4))
                    }
                    ...
                    if step >= 2 && !saltPoured {
                        Button {
                            ...
                            HStack(spacing: 4) {
                                ...
                                Text("Pour Salt Water").font(.custom("EBGaramond-SemiBold", size: 12))
                            }
                            .foregroundStyle(seaBlue)
                            .padding(.horizontal, 12).padding(.vertical, 6)
                            .background(seaBlue.opacity(0.1)).cornerRadius(6)
                    ...
                    if step >= 3 {
                        FormulaText(text: "Soft + porous beats hard + dense at sea", highlighted: true, fontSize: 10)
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    HStack(spacing: w * 0.06) {
                        VStack(spacing: 4) {
                            Text("ROMAN").font(.custom("Cinzel-Bold", size: 9)).foregroundStyle(sepiaInk.opacity(0.5))
                            RoundedRectangle(cornerRadius: 4).fill(stoneGray.opacity(0.4 + years * 0.4))
                                .frame(width: w * 0.25, height: h * 0.25)
                            Text(years > 0.5 ? "Stronger" : "").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.sageGreen)
                        }
                        VStack(spacing: 4) {
                            Text("MODERN").font(.custom("Cinzel-Bold", size: 9)).foregroundStyle(sepiaInk.opacity(0.5))
                            ...
                            Text(years > 0.5 ? "Dissolves" : "").font(.custom("EBGaramond-Bold", size: 10)).foregroundStyle(RenaissanceColors.errorRed)
                        }
                    }.position(x: cx, y: h * 0.38)

                    Text("\(Int(years * 2000)) years").font(.custom("EBGaramond-Bold", size: 14)).foregroundStyle(sepiaInk).position(x: cx, y: h * 0.12)
                    ...
                    if step >= 3 { FormulaText(text: "Seawater = secret ingredient", highlighted: true, fontSize: 11).position(x: cx, y: h * 0.85).transition(.opacity) }
                }
            }
        }
    }
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ForEach(0..<layersAdded, id: \.self) { i in
                        ...
                        DimLabel(text: parts[i].0, fontSize: 9).position(x: cx + w * 0.32, y: h * 0.35 + CGFloat(i) * h * 0.12)
                    }
                    if step >= 2 && layersAdded < 3 {
                        ...
                        } label: { Text("Add \(parts[layersAdded].0)").font(.custom("EBGaramond-SemiBold", size: 12)).foregroundStyle(color).padding(.horizontal, 12).padding(.vertical, 6).background(color.opacity(0.1)).cornerRadius(6) }
                            .buttonStyle(.plain).position(x: cx, y: h * 0.82)
                    }
                    if step >= 3 { FormulaText(text: "Planks → Lead → Tacks", highlighted: true, fontSize: 11).position(x: cx, y: h * 0.85).transition(.opacity) }
                }
            }
        }
    }
        ...
        TeachingContainer(title: visual.title, color: color, totalSteps: 3, step: $step,
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    Text("\(tons) tons").font(.custom("EBGaramond-Bold", size: 14)).foregroundStyle(sepiaInk).position(x: cx, y: h * 0.12)
                    ...
                    if step >= 3 { FormulaText(text: "Oak + tannins = naturally waterproof", highlighted: true, fontSize: 11).position(x: cx, y: h*0.85).transition(.opacity) }
                }
            }
        }
    }
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    if step >= 2 && pilesHammered < 5 {
                        ...
                        } label: { HStack(spacing: 4) { Image(systemName: "arrow.down").font(.system(size: 10)); Text("Drive Pile").font(.custom("EBGaramond-SemiBold", size: 12)) }
                    ...
                    if step >= 3 { FormulaText(text: "Poplar swells tight — natural waterproof seal", highlighted: true, fontSize: 10).position(x: cx, y: h*0.88).transition(.opacity) }
                }
            }
        }
    }
...
private struct MarineMortarVisual: View {
    ...
    var body: some View {
        ZStack {
            ...
            IVBlueprintGrid()
            VStack(spacing: 10) {
                FormulaText(text: "1 Lime : 3 Volcanic Ash + Seawater", highlighted: seawaterAdded, fontSize: 13).padding(.top, 8)
                HStack(spacing: 12) {
                    Button { guard scoops == 0 else { return }; withAnimation(.spring(response: 0.3)) { scoops = 1 }; SoundManager.shared.play(.tapSoft) } label: {
                        Text("+ Lime").font(.custom("EBGaramond-SemiBold", size: 12)).padding(.horizontal, 12).padding(.vertical, 6)
                    ...
                    Button { guard scoops >= 1 && scoops < 4 else { return }; withAnimation(.spring(response: 0.3)) { scoops += 1 }; SoundManager.shared.play(.tapSoft) } label: {
                        Text("+ Ash").font(.custom("EBGaramond-SemiBold", size: 12)).padding(.horizontal, 12).padding(.vertical, 6)
                ...
                if scoops >= 4 && !seawaterAdded {
                    Button { withAnimation(.spring(response: 0.3)) { seawaterAdded = true }; SoundManager.shared.play(.correctChime) } label: {
                        HStack(spacing: 4) { Image(systemName: "drop.fill").font(.system(size: 10)); Text("Add SEAWATER (not fresh!)").font(.custom("EBGaramond-SemiBold", size: 12)) }
                            .foregroundStyle(seaBlue).padding(.horizontal, 14).padding(.vertical, 8).background(seaBlue.opacity(0.1)).cornerRadius(8)
                    }.buttonStyle(.plain)
                }
                if seawaterAdded { FormulaText(text: "Sets underwater in 7 days", highlighted: true, fontSize: 12) }
                Text(scoops < 1 ? "Add lime first" : scoops < 4 ? "Add ash (3 scoops)" : !seawaterAdded ? "Now the secret: SEAWATER" : "Salt triggers the reaction")
                    .font(.custom("EBGaramond-Regular", size: 10)).foregroundStyle(sepiaInk.opacity(0.6))
                Spacer()
        ...
        ZStack {
            ...
            VStack(spacing: 10) {
                ...
                Text(isMolten ? "Molten — pour into molds" : tempC > 200 ? "Warming..." : "Solid lead")
                    .font(.custom("EBGaramond-SemiBold", size: 11)).foregroundStyle(isMolten ? RenaissanceColors.sageGreen : sepiaInk)
                Slider(value: $temperature, in: 0...1).tint(isMolten ? RenaissanceColors.sageGreen : .orange).frame(width: 200)
                HStack { Text("0°C").font(.custom("EBGaramond-Regular", size: 8)); Spacer()
                    Text("327°C").font(.custom("EBGaramond-Regular", size: 8)).foregroundStyle(RenaissanceColors.sageGreen); Spacer()
                    Text("600°C").font(.custom("EBGaramond-Regular", size: 8))
                }.foregroundStyle(sepiaInk.opacity(0.4)).frame(width: 200)
                if isMolten { FormulaText(text: "Lowest melting point = most useful metal", highlighted: true, fontSize: 11) }
                Spacer()
            }.padding(.horizontal, 12)
        }.frame(height: height).clipShape(RoundedRectangle(cornerRadius: 10))
    }
