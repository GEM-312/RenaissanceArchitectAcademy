...
### DECIDED (Marina, 2026-09-20): give the existing `CityScene` a `ZoneDefinition` initializer. One class, one data struct. No base class, no per-zone subclasses.

`CityScene` stops hardcoding its map and instead reads everything map-specific from a `ZoneDefinition` passed at init. All six zones are the same class with different data.

This is still a **departure from the codebase's established convention** (full copy-paste per scene, confirmed in §0), and worth naming as such. The reason to depart: `CityScene.swift` is 1336 lines, of which roughly 700–800 (camera setup/clamp/pan/pinch/scroll, theme toggling, Dijkstra pathfinding, dark-mode glow, tree-sway math, editor-mode registration) are *generic* — they don't depend on which 2–8 buildings are on the map or what the terrain looks like. Copy-pasting six times means ~4500 lines that all have to be bug-fixed in lockstep forever (the walk-follow lerp comment at `CityScene.swift:351-357` is exactly the kind of subtle fix that would need to land in 6 places). The Workshop/Forest/CraftingRoom/Goldsmith precedent tolerates duplication because there are only 4, built one at a time over months; 6 new near-identical scenes at once is a different order of risk.

**An earlier draft of this plan proposed a `ZoneScene` base class with `CityScene` as a thin subclass. That was rejected** — it reached the same place with an extra class and an inheritance layer for no gain. §7 step 1 was always going to rewrite `CityScene`'s hardcoded data into a `ZoneDefinition` regardless; once that's done, a separate base class holds nothing the parameterized `CityScene` doesn't already hold. One class is closer to CLAUDE.md's "no abstractions beyond what was requested."

The name `CityScene` stays. Renaming to `ZoneScene` would churn `CityMapView.swift`, `SceneHolder<CityScene>` and the editor-mode plumbing for no behavioral gain; rename later if it ever reads wrong.
...
```swift
// Views/SpriteKit/CityScene.swift — EXISTING file, parameterized in place
/// Unchanged in behavior. Everything that used to be a hardcoded `private let`
/// describing THIS map — mapSize, waypoints, waypointEdges, buildingWaypoints,
/// the 17-building tuple array, romeTerrainTrees, the six addZoneLabel() calls,
/// the "Terrain"/"BlurredTerrain" names, the player spawn — now comes from
/// `zone`. Everything else (camera setup/clamp/pan/pinch/scroll, theme toggle,
/// Dijkstra, dark glow, editor mode) is untouched and already zone-agnostic.
///
/// Note one split worth getting right: `addSwayingTree(image:position:scale:)`
/// — the warp grid, the phase jitter, the walk-by disturbance — is generic and
/// stays as-is; the LIST of which trees a map has is per-zone data
/// (`zone.trees`), because each cut-out is lifted from its own zone's terrain.
class CityScene: SKScene, ScrollZoomable {
    ...
    // No other signature changes. Method bodies swap `mapSize` → `zone.mapSize`,
    // `waypoints` → `zone.waypoints`, and so on.
}
```

```swift
// Models/ZoneRegistry.swift — new file, the six definitions
enum ZoneRegistry {
    static let ancientRome = ZoneDefinition(id: "ancientRome", /* today's exact values */)
    static let florence    = ZoneDefinition(id: "florence",    /* … */)
    // …
    static func zone(for id: ZoneID) -> ZoneDefinition { … }
}
// Call sites become CityScene(zone: ZoneRegistry.ancientRome).
```

**The risk this carries, and how to hold it.** Retrofitting a working, shipped scene in place means there is no moment where the live Ancient Rome map is running untouched code — unlike the rejected base-class route, where `CityScene` stayed frozen until the final step. The mitigation is that step 1 is a **pure data move with no behavior change**: every value in `ZoneRegistry.ancientRome` must be byte-identical to the literal it replaces, so the diff is mechanically reviewable and the smoke test in §7 step 1 is a true regression test. Do not fold any cleanup, renaming or "while I'm in here" improvement into step 1.

**Why not six independent copy-pasted scenes (matching the Workshop/Forest/CraftingRoom convention exactly)?** Considered and rejected. It costs ~3000–4500 lines of near-duplicate code but zero new architecture, and it is what every other "new map" in this repo has done. Rejected because 6 copies of the same camera/pathfinding/sway code is a bug-fix trap, and because the parameterized route adds exactly one data struct — not a framework.
...
3. **Per-zone waypoint graph** → `ZoneDefinition.waypoints`/`waypointEdges`/`buildingWaypoints`, replacing `CityScene.swift:114-226`. `Dijkstra`/`findPath` itself is zone-agnostic and stays in `CityScene` unchanged.
...
- **New, small, zone-unlock state.** Add `var unlockedZoneIds: Set<String>` to `CityViewModel` (mirrors the existing `earnedScienceBadges: Set<Science>` pattern at `:8`), seeded with `["ancientRome"]`. Gate `CityScene(zone:)` construction / the travel-map's tap target on membership in this set.
...
6. **Remove the now-dead code**: step 1 already deletes the hardcoded 17-building list, 40-waypoint array, tree array and `mapSize` from `CityScene.swift` (they move into `ZoneRegistry.ancientRome`), so by this point the only cleanup left is the no-argument `CityScene()` path — confirm nothing still calls it from `CityMapView.swift:883` or elsewhere, then drop it so `init(zone:)` is the only way to build a scene. Also revisit `hiddenBuildingIds`: once every building lives on its own zone's terrain, suppressing buildings by ID is obsolete and the set should go.
...
2. **Memory from six terrains.** Today one `4500×3214` sharp + `2048×1463` blurred pair is resident whenever the city map scene exists; `willMove(from:)` (`CityScene.swift:326-339`) already calls `terrainBlur.cleanup()` and `removeAllChildren()` on scene teardown, so the existing pattern (one zone's terrain loaded at a time, freed on exit) should carry over directly *if* each zone truly gets its own `SKScene` instance that's torn down on zone switch — this needs to be an explicit design constraint on `TravelMapView`'s navigation (never keep more than one `CityScene` alive at once), not an incidental property, since `SceneHolder<T>` (`CityMapView.swift:114`) exists specifically to *keep* a scene alive across SwiftUI re-renders — that's the right behavior *within* a zone, but the wrong behavior *across* zones if not bounded correctly.
